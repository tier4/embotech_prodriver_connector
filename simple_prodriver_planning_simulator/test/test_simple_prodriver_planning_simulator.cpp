...
using simulation::simple_prodriver_planning_simulator::SimpleProdriverPlanningSimulator;
...
{
public:
  PubSubNode() : Node{"test_simple_prodriver_planning_simulator_pubsub"}
  {
...
class TestSimpleProdriverPlanningSimulator : public ::testing::TestWithParam<std::string>
{
};

TEST_P(TestSimpleProdriverPlanningSimulator, TestIdealSteerVel)
{
  ...
  const auto sim_node = std::make_shared<SimpleProdriverPlanningSimulator>(node_options);
...
INSTANTIATE_TEST_SUITE_P(
  TestForEachVehicleModel, TestSimpleProdriverPlanningSimulator, ::testing::ValuesIn(VEHICLE_MODEL_LIST));
