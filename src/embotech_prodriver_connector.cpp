...
: Node("embotech_prodriver_connector", options), tf_buffer_(get_clock()), tf_listener_(tf_buffer_)
{
  ...
  adaptor.init_srv(srv_clear_route_, this, &EmbotechProDriverConnector::on_clear_route);
  adaptor.init_srv(srv_set_route_, this, &EmbotechProDriverConnector::on_set_route);
  adaptor.init_srv(srv_set_route_points_, this, &EmbotechProDriverConnector::on_set_route_points);
  ...
  // initial route state: not set
  state_.stamp = now();
  state_.state = RouteState::Message::UNSET;
  pub_state_->publish(state_);
}

void EmbotechProDriverConnector::on_clear_route(
  const ClearRoute::Service::Request::SharedPtr, const ClearRoute::Service::Response::SharedPtr res)
{
  if(state_.state == RouteState::Message::SET) {
    ...
    ptcl_route.numElements = 0U;
    ptcl_route.numGoalStates = 0U;
...
void EmbotechProDriverConnector::on_set_route(
  ...
  const SetRoute::Service::Response::SharedPtr res)
{
  if (state_.state != RouteState::Message::UNSET) {
    throw component_interface_utils::ServiceException(
      autoware_adapi_v1_msgs::srv::SetRoute::Response::ERROR_ROUTE_EXISTS, "The route is already set.");
  }
  auto goal_pose = std::make_shared<PoseStamped>();
  goal_pose->header = req->header;
  goal_pose->pose.position = req->goal.position;
  on_goal(goal_pose);
  res->status.success = true;
}
void EmbotechProDriverConnector::on_set_route_points(
  const SetRoutePoints::Service::Request::SharedPtr,
  const SetRoutePoints::Service::Response::SharedPtr)
{
  throw component_interface_utils::ServiceException(
    autoware_adapi_v1_msgs::srv::SetRoutePoints::Response::ERROR_PLANNER_UNREADY, "Setting route points is not supported.");
}
...
void EmbotechProDriverConnector::on_goal(const PoseStamped::ConstSharedPtr msg)
{
  ...
  // publish an empty route and the SET state needed by the 'ad_service_state_monitor'
  Route::Message route;
  route.header = msg->header;
  route.start_pose = current_kinematics_->pose.pose;
  route.goal_pose = msg->pose;
  pub_route_->publish(route);
  state_.stamp = now();
  state_.state = RouteState::Message::SET;
  pub_state_->publish(state_);
}
